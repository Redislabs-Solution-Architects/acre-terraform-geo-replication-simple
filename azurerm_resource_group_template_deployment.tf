resource "azurerm_resource_group_template_deployment" "acre_cluster" {
  ...
  resource_group_name = random_string.resource_group_name
  deployment_mode     = var.acre_deployment_mode
  template_content = templatefile(var.acre_template_path_1,
    {
      acre_name_1          = format("%s-%s", random_string.acre_group_name, var.primary-region),
      acre_name_2          = format("%s-%s", random_string.acre_group_name, var.secondary-region),
      ...
      acre_group_nickname  = random_string.acre_group_name,

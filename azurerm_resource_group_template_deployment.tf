resource "azurerm_resource_group_template_deployment" "acre_cluster" {
  ...
  template_content = templatefile(var.acre_template_path_1,
    {
      acre_name_1          = format("%s-%s", random_string.acre_group_name.result, var.primary-region),
      acre_name_2          = format("%s-%s", random_string.acre_group_name.result, var.secondary-region),
